                                    ...
                                    <table class="table allcp-form theme-warning tc-checkbox-1 fs13">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">{{$emp->employee?$emp->employee->code:''}}</td>
                                                <td class="text-center">{{$emp->name}}</td>
                                                <td class="text-center">{{$emp->employee?convertStatusBack($emp->employee['status']):''}}
                                                </td>
                                                <td class="text-center">
                                                    {{isset($emp->role->role->name)?$emp->role->role->name:''}}</td>
                                                <td class="text-center">{{isset($emp->employee->date_of_joining)?date('Y-m-d',
                                                    strtotime($emp->employee['date_of_joining'])):''}}</td>
                                                {{-- <td class="text-center">{{$emp->employee->current_address}}</td> --}}
                                                <td class="text-center">{{isset($emp->employee->number)?$emp->employee->number:''}}</td>
                                                <td class="text-center">{{isset($emp->employee->department)?$emp->employee->department:''}}</td>
                                                <td class="text-center">
