                                    ...
                                    <table class="table allcp-form theme-warning tc-checkbox-1 fs13">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">{{(isset($post))? $leave->name :
                                                    $leave->user->name}}</td>
                                                <td class="text-center">{{(isset($post))? $leave->code :
                                                    $leave->user->employee->code}}</td>
                                                <td class="text-center">{{(isset($post))? $leave->leave_type :
                                                    getLeaveType($leave->leave_type_id)}}</td>
                                                <td class="text-center">{{getFormattedDate($leave->date_from)}}</td>
                                                <td class="text-center">{{getFormattedDate($leave->date_to)}}</td>
                                                <td class="text-center">{{$leave->days}}</td>
                                                <td class="text-center" id="remark-{{$leave->id}}">
                                                    {{(isset($leave->remarks)) ? $leave->remarks : 'N/A'}}</td>
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group text-right" id="button-{{$leave->id}}">
                                                        ...
                                                        <ul class="dropdown-menu" role="menu">
                                                            <li>
                                                                <a class="approveClick" data-id="{{$leave->id}}"
                                                                    data-name="approve">Approve</a>
                                                            </li>
                                                            <li>
                                                                <a class="disapproveClick" data-id="{{$leave->id}}"
                                                                    data-name="disapprove">Disapprove</a>
                                                            </li>
                                                        </ul>
                                                        @elseif($leave->status==1)
                                                        <button type="button" class="btn btn-success br2 btn-xs fs12"
                                                            aria-expanded="false"><i class="fa fa-check"> Approved </i>

                                                        </button>
                                                        @else
                                                        <button type="button" class="btn btn-danger br2 btn-xs fs12"
                                                            aria-expanded="false"> <i class="fa fa-times"> Disapproved
                                                            </i>
